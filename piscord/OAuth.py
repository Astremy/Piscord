from .API_Elements import Member, Guild, User
import json
...
class OAuth:
    ...
    def add_guild_member(self, token, guild_id, user_id):
        if "guilds.join" in self.scope:
            return Member({**self.bot.api_call(f"/guilds/{guild_id}/members/{user_id}", "PUT", json=json.loads(token)["access_token"]), "guild_id": guild_id}, self.bot)
        return "Invalid Scope"
