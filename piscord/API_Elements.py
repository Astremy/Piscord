        ...
        for x, y in self.__dict__.items():
            ...
            if y is not None:
                if type(y) == list:
                    e = []
                    for p in y:
                        if isinstance(p, APIElement):
                            p = p.to_json()
                        e += [p]
                    y = e
                if isinstance(y, APIElement):
                    y = y.to_json()
                output[x] = y
        return output
...
class Guild:
    ...
    def create_role(self, **kwargs):
        ...
        return Role({**self.__bot.api(f"/guilds/{self.id}/roles", "POST", json=kwargs), "guild_id": self.id},
                    self.__bot)

    def count_prune(self, days=7, include_roles=None):
    ...
    def prune(self, days=7, include_roles=None):
...
class Channel:
    ...
    def __init__(self, channel, bot, guild=None):
        ...
        self.permission_overwrites = [Overwrite(overwrite, bot, self.id) for overwrite in
    ...
    def send(self, content=None, files=None, **kwargs):
        ...
        return Message(
            self.__bot.api(f"/channels/{self.id}/messages", "POST", json={"content": content, **kwargs}),
            self.__bot)
    ...
    def create_webhook(self, name, avatar=None):
        ...
        return Webhook(self.__bot.api(f"/channels/{self.id}/webhooks", "POST", json={"name": name, "avatar": avatar}),
                       self.__bot, channel=self)
    ...
    def purge(self, max_, before=None, after=None):
        ...
        messages = [message.id for message in self.get_messages(limit=max_, before=before, after=after)]
...
class AllowedMentions(APIElement):
    """
...
class Embed(APIElement):
    ...
    type:
        ...
        Embed types should be considered deprecated and might be removed
            ...
            - "image" : Image Embed
            - "video" : Video Embed
            - "gif" : Animated gif image Embed rendered as a video Embed
    ...
    def __init__(self, embed=None):
        ...
        self.footer = EmbedFooter(embed.get("footer", {}))
        self.image = EmbedImage(embed.get("image", {}))
        self.thumbnail = EmbedImage(embed.get("thumbnail", {}))
        self.video = EmbedImage(embed.get("video", {}))
        self.provider = EmbedProvider(embed.get("provider", {}))
        self.author = EmbedAuthor(embed.get("author", {}))
        self.fields = [EmbedField(field) for field in embed.get("fields", [])]
    ...
    def add_field(self, **kwargs):
        ...
        self.fields.append(EmbedField(kwargs))
        return self.fields[-1]


class EmbedFooter(APIElement):
    """
...
class EmbedProvider(APIElement):
    """
