...
class Bot_Element:
    ...
    def __init__(self, bot_element, bot):
        ...
        self.guilds = [Guild(guild, bot)
                       for guild in bot_element.get("guilds", [])]
...
class Guild:
    ...
    def __init__(self, guild, bot):
        ...
        self.default_message_notifications = guild.get(
        ...
        self.members = [Member(member, bot)
                        for member in guild.get("members", [])]
        self.channels = [Channel(channel, bot, guild=self)
        ...
        self.premium_subscription_count = guild.get(
        ...
        self.approximate_presence_count = guild.get(
    ...
    def get_members(self, limit=100, after=0):
        members = self.__bot.api(
            f"/guilds/{self.id}/members", "GET", params={"limit": limit, "after": after})
...
class Channel:
    ...
    def __init__(self, channel, bot, guild=None):
        ...
        self.recipients = [User(user, bot)
                           for user in channel.get("recipients", [])]
...
class Message:
    ...
    def __init__(self, message, bot):
        ...
        if "member" in message:
            self.author = Member(
                {**message["member"], "user": {**message["author"]}, "guild_id": self.guild_id}, bot)
    ...
    def edit(self, **modifs):
        self.__bot.api(
            f"/channels/{self.channel_id}/messages/{self.id}", "PATCH", json=modifs)

    def add_reaction(self, reaction):
        self.__bot.api(
            f"/channels/{self.channel_id}/messages/{self.id}/reactions/{reaction}/@me", "PUT")

    def delete_reactions(self):
        self.__bot.api(
            f"/channels/{self.channel_id}/messages/{self.id}/reactions", "DELETE")

    def delete_self_reaction(self, reaction):
        self.__bot.api(
            f"/channels/{self.channel_id}/messages/{self.id}/reactions/{reaction}/@me", "DELETE")

    def delete_reaction(self, reaction, user_id=None):
        if isinstance(reaction, Emoji):
            reaction = reaction.get_reaction()
        if user_id:
            self.__bot.api(
                f"/channels/{self.channel_id}/messages/{self.id}/reactions/{reaction}/{user_id}", "DELETE")
        else:
            self.__bot.api(
                f"/channels/{self.channel_id}/messages/{self.id}/reactions/{reaction}", "DELETE")
...
class Member(User):
    ...
    def edit(self, **modifs):
        if hasattr(self, "id"):
            user_id = self.id
            self.__bot.api(
                f"/channels/{self.guild_id}/messages/{user_id}", "PATCH", json=modifs)

    def kick(self):
        if hasattr(self, "id"):
            user_id = self.id
            delete_member = self.__bot.api(
                f"/guilds/{self.guild_id}/members/{user_id}", "DELETE")

    def ban(self, reason=None):
        if hasattr(self, "id"):
            user_id = self.id
            self.__bot.api(
                f"/guilds/{self.guild_id}/bans/{user_id}", "PUT", json={"reason": reason})

    def add_role(self, role):
        if hasattr(self, "id"):
            user_id = self.id
            self.__bot.api(
                f"/guilds/{self.guild_id}/members/{user_id}/roles/{role.id}", "PUT")

    def remove_role(self, role):
        if hasattr(self, "id"):
            user_id = self.id
            self.__bot.api(
                f"/guilds/{self.guild_id}/members/{user_id}/roles/{role.id}", "DELETE")
...
class Role:
    ...
    def edit(self, **modifs):
        self.__bot.api(
            f"/channels/{self.guild_id}/messages/{self.id}", "PATCH", json=modifs)
...
class Invite:
    ...
    def __init__(self, invite, bot):
        ...
        self.approximate_presence_count = invite.get(
            "approximate_presence_count")
...
class Overwrite(API_Element):
    ...
    def edit(self, **modifs):
        self.__bot.api(
            f"/channels/{self.channel_id}/permissions/{self.id}", "PUT", json=modifs)

    def delete(self):
        self.__bot.api(
            f"/channels/{self.channel_id}/permissions/{self.id}", "DELETE")
